...
export class NetworkController extends BaseController<NetworkConfig, NetworkState> {
  ...
  private refreshNetwork() {
    ...
    const { rpcTarget, type, chainId, ticker, nickname } = this.state.provider;
    console.log("refreshNetwork", rpcTarget, type, chainId, ticker, nickname);
  ...
  setProviderType(type: NetworkType) {
    ...
    this.update({
      provider: {
        ...providerState,
        ...{ type, ticker: 'ETH', rpcTarget, chainId, nickname },
      },
    });
    this.refreshNetwork();
  }
